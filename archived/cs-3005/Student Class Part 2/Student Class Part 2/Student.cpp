...
bool Student::operator<(const Student &rhs) const
{
    ...
    for(i = 0; i < 11;) {
        if(lhs_social[i] < rhs_social[i]) {
            return true;
        } else if(lhs_social[i] > rhs_social[i]) {
            return false;
        }
        if(lhs_social[i] == rhs_social[i]) {
            i++;
...
std::ostream &operator<<(std::ostream &os, const Student &rhs)
{
    ...
    if(last_name == 0) { last_name = "NONE"; }
    if(first_name == 0) { first_name = "NONE"; }
